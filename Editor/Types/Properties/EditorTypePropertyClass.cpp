...
void EditorTypePropertyClass::DrawImGUI()
{
	...
	else
	{
		...
		{
			std::vector<std::weak_ptr<EditorAssetClass>> validAssets = Game::GetAssetManager().GatherAssetsOfClass(className, true);
...
void* EditorTypePropertyClass::GetValue() const
{
	...
	{
		return Game::GetAssetManager().LoadObjectFromClassAssetName<void>(assetName);
	}
	return nullptr;
}
