...
void EditorTypePropertiesBase::AddAndSetupProperty(EditorTypePropertyBase* pProperty, int insertIndex/*= -1*/)
{
	pProperty->onPropertyChanged.Add(onPropertyChangedDelegate);
	...
	{
		pProperties.insert(pProperties.begin() + insertIndex, pProperty);
	}
}
...
void EditorTypePropertiesBase::RemoveProperty(EditorTypePropertyBase* pProperty, bool bDeleteProperty)
{
	...
	{
		DOMLOG_ERROR("Tried to remove property that doesn't exist", pProperty->GetName());
	}
}
